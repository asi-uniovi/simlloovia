...
colors = [
    'rgb(229,236,246)', 'hsl(280, 100, 85)', app_exec_color[0],
    'rgb(229,236,246)', 'hsl(40, 100, 85)', app_exec_color[1],
    'rgb(229,236,246)', 'hsl(20, 100, 85)', app_exec_color[2],
    'rgb(229,236,246)', 'hsl(150, 100, 85)', app_exec_color[3],
    ]
...
app.layout = html.Div(children=[
    ...
    html.Div([
        ...
        html.Span(id='req-selector', children=[
                " First request: ",
...
tab_summary = html.Div([
    dcc.Loading(
            ...
            children=[
                html.Div([
                    html.Div(dash_table.DataTable(id='req-table',
                        style_as_list_view=True, style_header={ 'display': 'none', },
                        ), className="two columns"),
                    html.Div(dash_table.DataTable(id='sim-table',
                        style_as_list_view=True, style_header={ 'display': 'none', },
                        ), className="two columns"),
                    html.Div(dash_table.DataTable(id='sol-table',
                        style_as_list_view=True, style_header={ 'display': 'none', },
...
tab_detail = html.Div([
    ...
    dcc.Loading(
        ...
        type="default",
        children=[
            html.Div(dash_table.DataTable(id='detail-table',
                export_format='csv')),
        ]
    ),
])
...
tab_util = html.Div([
    ...
    dcc.Loading(
        ...
        type="default",
        children=[
            html.Div(dash_table.DataTable(id='table-util',
...
def apply_new_dir_gantt(n_clicks, gantt_type, n_clicks_app_filter, base_dir,
        prefix, first_req, last_req, app_filter):
    try:
        first_req = int(first_req)
        last_req = int(last_req)

        return get_gantt(gantt_type, base_dir, prefix, first_req, last_req,
            app_filter),
    except Exception as e:
...
def update_detail_table_and_plot(n_clicks, aggregations, agg_type, plot,
        base_dir, prefix, first_req, last_req):
    data = None
    cols = None
    children = None
    ...
    try:
        ...
        df_table = df_req
        if aggregations:
            df_table = df_table.groupby(aggregations).agg(
                    {'req': 'count', 'resp_time': agg_type, 'serv_time': 'mean'}
                ).reset_index()
    ...
    if plot:
        try:
            children = get_resp_time_plot(df_req, df_table, aggregations,
                agg_type)
        except Exception as e:
...
def get_summary(base_dir, prefix):
    df = pd.read_csv(f'{base_dir}/{prefix}.csv',
            names=['key', 'value'], header=0, index_col=0).T

    req_df = df[['req_injected', 'req_proc', 'req_pending', 'req_lost',
        'avg_resp_time', 'max_resp_time', 'min_resp_time']].T.reset_index()

    sim_df = df[['cost', 'util', 'sim_time', 'stats_time']].T.reset_index()

    sol_df = df[['sol_file', 'workload', 'workload_period',
        'workload_length']].T.reset_index()

    res_data = req_df.to_dict('rows')
    sim_data = sim_df.to_dict('rows')
    sol_data = sol_df.to_dict('rows')

    res_cols = dash_cols(req_df)
    sim_cols = dash_cols(sim_df)
    sol_cols = dash_cols(sol_df)

    req_info_df = df[['req_injected', 'req_proc', 'req_pending',
        'req_lost']].T.reset_index()
    fig_req = px.bar(req_info_df, x='key', y='value')
    fig_req.update_yaxes(title="Number of requests")
    fig_req.update_xaxes(title="")

    resp_time_df = df[
            ['min_resp_time', 'avg_resp_time', 'max_resp_time',]
        ].T.reset_index()
    ...
    return (res_data, res_cols, sim_data, sim_cols, sol_data, sol_cols, fig_req,
        fig_resp_time, cost, wl_length_h)
...
def get_gantt(gantt_type, base_dir, prefix, first_req, last_req, app_filter):
    ...
    df_req['req_app'] = df_req.groupby(gantt_type)['req'].rank(method='first', ascending=True)
    df_req['real_req_app'] = df_req.groupby('app')['req'].rank(method='first', ascending=True)

    # Copy all requests and change the type to "Resp" so the waiting and the
    ...
    df_all = df_service.append(df_req)
    ...
    for i in plot_order:
        ...
        gantt = px.timeline(df_i, x_start='x_start', x_end='x_end', y='req_app',
            color='type', color_discrete_sequence=app_exec_color,
...
def apply_new_dir_util(n_clicks, base_dir, prefix, first_req, last_req):
    ...
    return fig, df.to_dict('rows'), dash_cols(df)
...
@click.command()
@click.option('--port', type=int, required=False, help='Port to launch the web app', default=8050)
